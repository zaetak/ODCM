...
Partial Class Orthodontics
    Inherits System.Web.UI.Page

    Public Sub LoadPatientTeethInfo()
        SqlQuery("select a.teethid,concat(b.lastname,', ',b.firstname,' ',b.middlename) as name,a.service,DATE_FORMAT(a.examdate,'%m/%d/%Y'),a.chiefcomplaint,a.dentalhistory,a.medication,a.congenital,a.temporo,a.oralhygiene,a.gingival,a.diagnosis from patientteeth a, clienttbl b where a.clientid=b.clientid and a.id='" & Session("PatientID") & "'")
    ...
    Protected Sub RadButton1_Click(sender As Object, e As EventArgs) Handles RadButton1.Click
        SqlQuery("insert into patientteeth(clientid,teethid,service,examdate,diagnosis,chiefcomplaint,dentalhistory,medication,congenital,temporo,oralhygiene,gingival) values('" & Session("clientid") & "','" & Teethid.Text & "','" & Service.Text & "','" & Format(RadDatePicker1.SelectedDate, "yyyy-MM-dd") & "','" & RadTextBox2.Text & "','" & chiefcomplaint.Text & "','" & dentalhistory.Text & "','" & medication & "','" & congenital & "','" & temporo & "','" & oralhygiene & "','" & gingival & "')")
        RadWindowManager1.RadAlert("Added Successfully.", 330, 180, "DENTCAST", "callBackFn", "/Images/Success.png")
    End Sub

    Public Sub Clear()
        SqlQuery("Select concat(lastname,', ',firstname,' ',middlename) as name from clienttbl where clientid='" & Page.Session("LoginID") & "'")
    ...
    Private Sub Orthodontics_Load(sender As Object, e As EventArgs) Handles Me.Load
        If IsPostBack = False Then
            LoadService()
            If Session("PatientID") <> Nothing Then
